...
namespace TesoTellerIntDevice
{
    public class BpIdentity
    {
        ...
        private static int bpWorkStyle=0;
          ...
          //  pBPCtrl[3] = (byte)(nBaudRate & 0xff); 
            ...
            try
            {
               ...
               // nBPHandle = FingerReaderDllWrapper.TcOpenComm(nCommPort, 3, 0, 0);
                ...
                FingerReaderDllWrapper.WritComm(nHdlPort, pBPCtrl, 4, false);
            } catch (Exception) {
                return -1;
            }
            finally
        ...
        // 暂时不理会转口指令
        public static int CommPortToInt(String StrPortName)
        {
            int nTmpPort=0;
